...
namespace Gen {
	Generator::Generator(LT::LexTable plexT, IT::IdTable pidT, wchar_t pout[])//констр (из стр-зел  генератор берем констр генератор)
	{
		lexT = plexT;
		idT = pidT;
		out = ofstream(pout, ios_base::out);
		//выводят текст в файл аутпут.асм
		Head();//заголовок 
		Const();// 
		Data();// переменные в языке 
		Code();
	}
	...
	void Generator::Head() {
		...
		out << "includelib ../MIV-2021/Debug/StaticLibrary.lib\n";
